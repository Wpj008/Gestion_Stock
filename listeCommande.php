...
$usercommande = $data->prepare("SELECT * FROM utilisateurs INNER JOIN commandes ON utilisateurs.id = commandes.utilisateur_id");

$usercommande -> execute();

$usersCOMMANDE = $usercommande->fetchAll();

foreach($usersCOMMANDE as $usersCOMMANDES){

    $user_commande = $usersCOMMANDES['nom'];
}
...
$query = $data->prepare("SELECT * FROM produits INNER JOIN commandes ON produits.id = commandes.produit_id ");
